 ...
 * ersion of cooperative coevolution: only one subpopulation is evaluated and bred per generation.
 * he subpopulation index to breed is determined by taking the generation number, modulo the
 * otal number of subpopulations.
 * <p>
 * p><b>Parameters</b><br>
 * <table>
                ...
                for (int k = 0; k < guruIndividuals[j].length; k++) {
                    for (int ind = 0; ind < inds.length; ind++) {
                        if (ind == j) {
                            ...
                            inds[ind] = guruIndividuals[ind][k];
                            updates[ind] = false;
                        }
                    }
                    prob.evaluate(state, inds, updates, false, subpops, 0);
...
class GuruComparator implements SortComparatorL {
    ...
    public GuruComparator(ArrayList<Individual> inds) {
        super();
        this.inds = inds;
    }
