...
public class ThreadPool implements java.io.Serializable {
    ...
    public int getOutstandingWorkers() {
        synchronized (workersLock) {
            return getTotalWorkers() - getPooledWorkers();
        }
    }
            ...
            while (true) {
                synchronized (this) {
                    while (!go) {
                        ...
                        try {
                            wait();
                        }  // wait for a new job to work on, or a request to die
                        catch (InterruptedException e) {
                            Thread.interrupted();
                        } // ignore
                    }
                ...
                try {
                    ...
                    e.printStackTrace();
                    Thread.interrupted();
                } // resets interrupted flag.  Note ANY exception.
