...
public class SlaveMonitor {
    ...
    final boolean isShutdownInProgress() {
        synchronized (shutdownInProgressLock) {
    ...
    public boolean registerSlave(EvolutionState state, String name, Socket socket, DataOutputStream out, DataInputStream in) {
        ...
        {
            try {
                out.writeByte(Slave.V_SHUTDOWN);
            } catch (Exception e) {
            }  // exception, not IOException, because JZLib throws some array exceptions
            try {
                out.flush();
            } catch (Exception e) {
            }
