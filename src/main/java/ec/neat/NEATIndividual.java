...
import ec.neat.NEATNode.NodePlace;
import ec.neat.NEATNode.NodeType;
import ec.neat.NEATSpecies.MutationType;
import ec.util.Code;
        ...
        while (tryCount < newLinkTries) {
            ...
            for (int i = 0; i < genome.length; ++i) {
                ...
                if (gene.inNodeId == firstNode.nodeId && gene.outNodeId == secondNode.nodeId && gene.isRecurrent
                        && doRecur) {// already have a recurrent link between these nodes in
                    // recurrent case
                    bypass = true;
                    break;
                }
                if (gene.inNodeId == firstNode.nodeId && gene.outNodeId == secondNode.nodeId && !gene.isRecurrent
                        && !doRecur) {// already have a normal link between these nodes in normal
                    // case
                    bypass = true;
                    break;
                }
