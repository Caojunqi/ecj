        ...
        synchronized (mailbox.immigrants) {
            for (int x = 0; x < mailbox.immigrants.length; x++) {
                if (mailbox.nImmigrants[x] > 0) {
                    if (chatty)
                        state.output.message("Immigrating " + mailbox.nImmigrants[x] + " individuals from mailbox for subpopulation " + x);
                ...
                if (running[x]) {
                    try {
                        ...
                        while (true) {
                            ...
                            if (chatty)
                                state.output.message("Receiving " + how_many_to_come + " immigrants for subpopulation " + subpop + " from island " + incomingIds[x]);
